defmodule ExMon.PokeApi.ClientTest do
  use ExUnit.Case
  import Tesla.Mock

  alias ExMon.PokeApi.Client

  describe("get_pokemon/1") do
    test "when there is a pokemon with the given name, returns the pokemon" do
      # quando existe um pokemon com o nome dado, retorna o pokemon
      body = %{"name" => "pikachu", "weight" => 60, "types" => ["eletric"]}

      mock(fn %{method: :get, url: "https://pokeapi.co/api/v2/pokemon/pikachu"} ->
        %Tesla.Env{status: 200, body: body}
      end)

      response = Client.get_pokemon("pikachu")

      expected_response = {:ok, %{"name" => "pikachu", "types" => ["eletric"], "weight" => 60}}

      assert response == expected_response
    end

    test "when there no pokemon with the given name, returns the error" do
      # quando não há pokemon com o nome dado, retorna o erro
      mock(fn %{method: :get, url: "https://pokeapi.co/api/v2/pokemon/trovao"} ->
        %Tesla.Env{status: 404}
      end)

      response = Client.get_pokemon("trovao")

      expected_response = {:error, "Pokemon not found!"}

      assert response == expected_response
    end

    test "when there is an unexpected error, returns the error" do
      # quando há um erro inesperado, retorna o erro
      mock(fn %{method: :get, url: "https://pokeapi.co/api/v2/pokemon/pikachu"} ->
        {:error, :timeout}
      end)

      response = Client.get_pokemon("pikachu")

      expected_response = {:error, :timeout}

      assert response == expected_response
    end
  end
end
